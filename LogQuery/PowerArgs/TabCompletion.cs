...
namespace LogQuery.PowerArgs
{
    ...
    public static class ConsoleHelper
    {
        ...
        private static string[] GetArgs(List<char> chars)
        {
            ...
            return (from t in ret where string.IsNullOrEmpty(t) == false select t.Trim()).ToArray();
        }
            ...
            while (true)
            {
                ...
                else if (info.Key == ConsoleKey.Tab)
                {
                   ...
                   /* foreach (var completionSource in tabCompletionHooks)
                    {
                        if (completionSource.TryComplete(info.Modifiers(ConsoleModifiers.Shift),  token, out completion)) break;
                    }*/
